...
def register_search_functions(sdk, kv):
    ...
    async def adapt(input):
        ...
        recent = tag_exps[-10:]
        recent_keeps = sum(1 for e in recent if e.get("status") == "keep")
        recent_crashes = sum(1 for e in recent if e.get("status") == "crash")
        keep_rate = recent_keeps / len(recent)
        crash_rate = recent_crashes / len(recent)
    ...
    async def suggest(input):
        ...
        suggestions = build_suggestions(mode, underexplored, high_yield, near_misses, bpb_trend)
...
def build_suggestions(mode, underexplored, high_yield, near_misses, trend):
    suggestions = []
    if mode == "explore":
        if underexplored:
